...
export default function MarketCard({
  ...
  const priceChange =
    isSettled && market.closePrice != null
      ? market.closePrice - market.startPrice
      : null;
  const pricePct =
    priceChange != null && market.startPrice > 0
      ? (priceChange / market.startPrice) * 100
      : null;

  const handleExpire = useCallback(() => setExpired(true), []);
  const handleBetPlaced = useCallback(
    (b: number) => setAppBalance(b),
    [setAppBalance]
  );
      ...
      >
        ...
        {isOpen && (
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at 50% 0%, rgba(40,204,149,0.05) 0%, transparent 70%)"
            }}
          />
        )}
        ...
        <div
          className={clsx(
            "h-px",
            isOpen
              ? "bg-gradient-to-r from-transparent via-[#28cc95]/40 to-transparent"
              : wonBet
        ...
        <div className="p-5">
          ...
          <div className="flex items-start justify-between gap-3 mb-5">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-2">
                ...
                {/*  {isOpen && (
                  <span className="flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-lg" style={{ background: "rgba(40,204,149,0.1)", color: "#5dd9ab", border: "1px solid rgba(40,204,149,0.15)" }}>
                ...
                {isSettled && (
                  <span
                    ...
                    style={{
                      background:
                        market.direction === "UP" ? "rgba(34,197,94,0.1)"
                        : market.direction === "DRAW" ? "rgba(255,255,255,0.06)"
                        : "rgba(239,68,68,0.1)",
                      color: market.direction === "UP" ? "#4ade80" : market.direction === "DRAW" ? "rgba(255,255,255,0.5)" : "#f87171",
                      border: `1px solid ${market.direction === "UP" ? "rgba(34,197,94,0.2)" : market.direction === "DRAW" ? "rgba(255,255,255,0.1)" : "rgba(239,68,68,0.2)"}`
                    }}
                  >
                    {market.direction === "UP" ? <TrendingUp className="w-3 h-3" />
                      : market.direction === "DOWN" ? <TrendingDown className="w-3 h-3" />
                      : null}
                    {market.direction === "DRAW" ? "DRAW — Refunded" : `${market.direction} WON`}
                  </span>
                )}
              </div>
              <h3
            ...
            {isOpen && (
              <MarketTimer closeAt={market.closeAt} onExpire={handleExpire} />
            )}
          </div>
            ...
            >
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{
                    background: myBetDirection === "UP" ? "#22c55e" : "#ef4444"
                  }}
                />
                ...
                <span
                  className={clsx(
                    "text-xs font-bold",
                    myBetDirection === "UP" ? "text-green-400" : "text-red-400"
                  )}
                >
                  {myBetDirection}
                </span>
          ...
          {isOpen && (
            <button
              ...
              className={clsx(
                ...
                connected
                  ? "text-black hover:shadow-lg hover:shadow-[#28cc95]/20 active:scale-[0.98]"
          ...
          {!isOpen && !isSettled && (
            <div
              className="w-full py-2.5 rounded-2xl text-center text-white/25 text-xs font-medium"
              style={{
